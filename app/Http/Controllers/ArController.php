...
class ArController extends Controller
{
    ...
    {   
        ...
        ->join('customers as c','c.id','=','pv.id')
       ...
       return view('arbalance', compact('data','cust','product','po_id','date','total_amt','total_amount','items','bal_amt'));
       ...
       /* $name = isset($_GET['po_id'])?$users->toArray()[0]['name']:'';*/
        ...
        $total_amount =  $digit->format(round((int)$total_amt));
         $total_amount = ucwords($total_amount);
          ...
          $bal_amt1 = isset($_GET['po_id'])?$users->toArray()[0]['total_amount']:'';
          $bal_amt2 = isset($_GET['po_id'])?$users->toArray()[0]['pay_this_time']:'';
          $bal_amt = (round((int)$bal_amt1))-(round((int)$bal_amt2));

        // Fetch all records
         $response['data'] = $product;

       return view('arsearch', compact('data','cust','product','po_id','date','total_amt','total_amount','items','bal_amt'));
